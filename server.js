...
app.post("/balance", (request, response) => {
	console.log("Inserting balance: " + request.body.balance);

  MongoClient.connect(url, function(err, db) {
    if (err) throw err;
    var dbo = db.db("mydb");
    var myobj = { Balance: request.body.balance};
    dbo.collection("Bank").insertOne(myobj, function(err, res) {
      if (err) throw err;
...
// // delete preBalance
app.delete("/preBalance", (request, response) => {
	console.log("Deleting previous balance: " + request.body.preBalId);

  MongoClient.connect(url, function(err, db) {
    if (err) throw err;
    var dbo = db.db("mydb");
    var myquery = { _id: new ObjectID(request.body.preBalId) };
    dbo.collection("Bank").deleteOne(myquery, function(err, obj) {
      if (err) throw err;
